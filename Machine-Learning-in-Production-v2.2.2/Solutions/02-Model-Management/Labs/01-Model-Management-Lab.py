...
# MAGIC In the demo notebook, we built a custom `RF_with_preprocess` model class that uses a `preprocess_input(self, model_input)` helper function to automatically pre-processes the raw input it receives before passing that input into the trained model's `.predict()` function.
# MAGIC 
...
model_path =  f"{working_dir}/RF_with_postprocess/"
