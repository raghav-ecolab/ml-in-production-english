...
# MAGIC %md And to help us manage our streams better, we will make use of **`untilStreamIsReady()`**, **`stopAllStreams()`** and define the following, **`my_stream_name`**:

# COMMAND ----------

my_stream_name = "lesson03"
...
class StreamingModel(PythonModel):
...
try:
  model_path = (f"{working_dir}/ml-production/StreamingModel/").replace("dbfs:", "/dbfs")
  shutil.rmtree(model_path)
except:
  print("Cannot delete directory.")

# Instantiate the class using the trained model
model = StreamingModel(trained_rf=rf)
...
# MAGIC 
# MAGIC Finally, after confirming that your model works properly, apply it in parallel on all rows of `streaming_data`.
...
# MAGIC %md
# MAGIC Now continuously write `predictions_df` to a parquet file as they get created by the model.

# COMMAND ----------

# TODO
checkpoint_location = f"{working_dir}/ml-deployment/stream-lab3.checkpoint"
write_path = f"{working_dir}/ml-deployment/lab3-predictions"

(streaming_data
  .writeStream
  .queryName(my_stream_name)
  .FILL_IN
)
...
# MAGIC %md
# MAGIC Check that your predictions are indeed being written out to `write_path`.
...
try:
  print(spark.read.format("delta").load(write_path).count())
